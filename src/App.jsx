...
import {
  ...
  Upload,
  Settings,
  RotateCcw, CheckCircle2 as Settings,
  RotateCcw, CheckCircle2Icon
} from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    if (db && userId) {
      ...
      const unsubSettings,
  RotateCcw, CheckCircle2 = onSnapshot(
  ...
  const TransactionsView = ({ sellers, drivers, db, transactions }) => {
    const handleAddToReturns = async (item) => {
      try {
        const ref = doc(db, `/artifacts/${__app_id}/public/data/returns`, item.id);
        await setDoc(ref, {
        ...
        await addDoc(collection(db, `/artifacts/${__app_id}/public/data/history`), {
          ...
          docId: item.id,
          details: `Položka '${item.itemName}' přidána do sekce Vrácení starých motorů.`,
          timestamp: serverTimestamp(),
        });
        alert('Přidáno do sekce Vrácení starých motorů.');
      } catch (e) {
        console.error(e);
        alert('Nepodařilo se přidat do vrácení.');
      }
    };
      ...
      return (
          <div className={containerClass}>
              <div className={cardClass}>
                  <h2 className="text-2xl font-bold mb-4 text-gray-800 flex items-center">
                      <Settings,
  RotateCcw, CheckCircle2Icon className="mr-2 text-indigo-500" /> Nastavení aplikace
                  </h2>
          ...
          <div className="relative flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              <button onClick={() => setCurrentView(Views.SETTINGS)} className={`flex items-center px-3 py-2 text-sm font-medium rounded-xl transition-all duration-200 ${currentView === Views.SETTINGS ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-800 hover:bg-gray-200'}`}>
                <Settings,
  RotateCcw, CheckCircle2Icon className="h-5 w-5 mr-1" />
                Nastavení
              </button>
            </div>
          </div>
        ...
        <button onClick={() => setCurrentView(Views.RETURNS)} className={navButtonClass}>
          <RotateCcw className="inline mr-2 h-5 w-5" /> Vrácení
        </button>
      ...
      <main className="py-8">
        ...
        {currentView === Views.RETURNS && <ReturnsView returns={returns} db={db} />}
        {currentView === Views.SETTINGS && <Settings,
  RotateCcw, CheckCircle2View db={db} />}
      </main>
    </div>
  );
};
  ...
  const ReturnsView = ({ returns, db }) => {
    ...
    const formatPhone = (raw) => {
      ...
      if (s.startswith?.('420') && s.length>=12) {
        return '+420 ' + s.slice(-9).replace(/(\d{3})(?=\d)/g,'$1 ').trim();
      }
      return s.replace(/(\d{3})(?=\d)/g,'$1 ').trim();
    };
    const formatCZK0 = (n) => Number(n||0).toLocaleString("cs-CZ", { minimumFractionDigits: 0, maximumFractionDigits: 0 });

    const list = returns.filter(r => (showCompleted ? r.returned : !r.returned));

    const mark = async (r) => {
      try {
        const ref = doc(db, `/artifacts/${__app_id}/public/data/returns`, r.id);
        await updateDoc(ref, { returned: !r.returned, returnedAt: serverTimestamp() });
        ...
        <div className={cardClass}>
          ...
          <div className="mb-4">
            <label className="inline-flex items-center gap-2 text-sm text-gray-600">
              ...
              <span>{showCompleted ? 'Zobrazit vrácené' : 'Zobrazit nevyřízené'}</span>
            </label>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className={tableClass}>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {list.map((r) => (
                  <tr key={r.id} className={r.returned ? 'opacity-60' : ''}>
                    <td className={tdClass}>{r.itemName}</td>
                    <td className={tdClass}>{r.note || '-'}</td>
                    <td className={tdClass}>{r.seller || '-'}</td>
                    <td className={tdClass}>{formatCZK0(r.sellingPriceCzk)}</td>
                    <td className={tdClass}>{r.deliveryCity || '-'}</td>
                    <td className={tdClass}>{r.customerAddress || '-'}</td>
                    <td className={tdClass}>
                      <div>{formatPhone(r.customerContact)}</div>
                      {r.customerPhone2 ? <div className="text-xs text-gray-600">{formatPhone(r.customerPhone2)}</div> : null}
                    </td>
                    <td className={tdClass}>
                      <button onClick={() => mark(r)} className={r.returned ? "text-emerald-600" : "text-gray-600"}>
                        {r.returned ? <><CheckCircle2 className="inline h-4 w-4 mr-1" /> Vráceno</> : 'Označit jako vráceno'}
                      </button>
                    </td>
                  </tr>
                ))}
